...
<script>
	jQuery(document).ready(function ($) {
		$("#dialog_todo").kendoWindow({
			width: "86%",
			height: "80%",
			title: "<?php _e('Add todo for Customer:','wp-smart-crm-invoices-free') ?>",
			visible: false,
			modal: true,
			draggable: false,
			resizable:false,
			...
			close: function () {
				...
				this.title("<?php _e('Add todo for Customer:','wp-smart-crm-invoices-free') ?>");
				$('.modal_loader').hide();
			}
		})
			...
			$.ajax({
				...
				success: function (response) {
					if (opener == "clienti") {//ricarico la grid solo se aperto da form clienti
						...
						noty({
							text: "<?php _e('TODO has been added','wp-smart-crm-invoices-free')?>",
							layout: 'center',
							type: 'success',
